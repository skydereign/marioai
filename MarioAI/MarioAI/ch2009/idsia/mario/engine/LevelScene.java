package ch2009.idsia.mario.engine;

import ch2009.idsia.mario.engine.level.BgLevelGenerator;
import ch2009.idsia.mario.engine.level.Level;
import ch2009.idsia.mario.engine.level.LevelGenerator;
import ch2009.idsia.mario.engine.level.SpriteTemplate;
import ch2009.idsia.mario.engine.sprites.*;
import ch2009.idsia.mario.environments.Environment;
import ch2009.idsia.utils.MathX;
...
//        level = LevelGenerator.createLevel(320, 15, levelSeed);
        if(GlobalOptions.level == null) {
        	level = LevelGenerator.createLevel(levelLength, 15, levelSeed, levelDifficulty, levelType);
        } else {
        	//level = LevelGenerator.createLevel(levelLength, 15, levelSeed, levelDifficulty, levelType);
        	level = new Level(GlobalOptions.level.getWidth(), GlobalOptions.level.getHeight());
        	System.out.println("Using custom level");
        	GlobalOptions.level.convert(level);
        	LevelGenerator.fixWalls(level);
        }
